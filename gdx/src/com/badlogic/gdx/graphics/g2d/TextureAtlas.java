...
public class TextureAtlas implements Disposable {
	 ...
	 * parent directory of the pack file to find the page
	 * images.
	 */
	...
	public void load(TextureAtlasData data) {
		loadTextures(data.pages);
		loadRegions(data.regions);
	}

	private void loadTextures(List<Page> pages) {
		textures.ensureCapacity(pages.size());
		for (Page page : pages) {
	...
	private void loadRegions(List<Region> regions) {
		...
		for (Region region : regions) {
			AtlasRegion atlasRegion = createAtlasRegion(region);
			this.regions.add(atlasRegion);
		}
	}

	private AtlasRegion createAtlasRegion(Region region) {
	 ...
	 * string comparison to find the region, so the
	 ...
	 * uses string comparison to find the region, so
	 ...
	 * {@link AtlasRegion#index index}. This method
	 * uses string comparison to find the regions, so the result should be cached
	 * rather than calling this method multiple
	 * times.
	 */
	 ...
	 * for each region, so the result should be
	 ...
	 * whitespace was stripped from the region when it was
	 * packed, the sprite is automatically positioned as if whitespace had not been
	 * stripped. This method uses string comparison to
	 * find the region and constructs a new sprite, so the result should be cached
	 * rather than calling this method multiple
	 * times.
	 */
	 ...
	 * This method uses string comparison to find
	 * the region and constructs a new sprite, so the result should be cached rather
	 ...
	 * to largest {@link AtlasRegion#index index}.
	 * This method uses string comparison to find the regions and constructs new
	 * sprites, so the result should be cached rather
	 * than calling this method multiple times.
	 * 
	 * @see #createSprite(String)
	 */
	 ...
	 * {@link NinePatch}. The region must have been packed with
	 * ninepatch splits. This method uses string comparison to find the region and
	 * constructs a new ninepatch, so the result should
	 ...
	 * releases all the textures backing all
	 * TextureRegions and Sprites, which should no longer be used after calling
	 * dispose.
	 */
	public void dispose() {
	...
	static public class TextureAtlasData {
		...
		public void load(FileHandle packFile, FileHandle imagesDir, boolean flip) {
			...
			ObjectMap<String, Field<Page>> pageFields = initializePageFields(entry);
			ObjectMap<String, Field<Region>> regionFields = initializeRegionFields(entry);

			final boolean[] hasIndexes = { false };

			try (BufferedReader reader = packFile.reader(1024)) {
				processEntries(reader, pageFields, regionFields, imagesDir, flip, hasIndexes, entry);
			} catch (Exception ex) {
		...
		private ObjectMap<String, Field<Page>> initializePageFields(final String[] entry) {
		...
		private ObjectMap<String, Field<Region>> initializeRegionFields(final String[] entry) {
		...
		private void processEntries(BufferedReader reader, ObjectMap<String, Field<Page>> pageFields,
				ObjectMap<String, Field<Region>> regionFields, FileHandle imagesDir,
				boolean flip, boolean[] hasIndexes, final String[] entry) throws IOException {
			String line = reader.readLine();

			skipEmptyLines(reader, line);
			...
			while (line != null) {
				if (line.trim().isEmpty()) {
					...
					page = processPageEntry(line, reader, pageFields, entry, imagesDir);
				} else {
					processRegionEntry(line, reader, regionFields, entry, page, flip, names, values);
				}
				line = reader.readLine();
			}
		}

		private void skipEmptyLines(BufferedReader reader, String line) throws IOException {
			while (line != null && line.trim().isEmpty()) {
				line = reader.readLine();
			}
		}

		private Page processPageEntry(String line, BufferedReader reader, ObjectMap<String, Field<Page>> pageFields,
				final String[] entry, FileHandle imagesDir) throws IOException {
			Page page = new Page();
			page.name = line;
		...
		private void processRegionEntry(String line, BufferedReader reader, ObjectMap<String, Field<Region>> regionFields,
				final String[] entry, Page page, boolean flip,
				Array<Object> names, Array<Object> values) throws IOException {
			...
			while (true) {
				...
				if (field != null) {
					...
					handleUnknownRegionField(entry, names, values, count);
				}
			}
			finalizeRegion(region, names, values);
		}

		private void handleUnknownRegionField(final String[] entry, Array<Object> names, Array<Object> values, int count) {
			if (names == null) {
				names = new Array<>(8);
				values = new Array<>(8);
			}
		...
		private void finalizeRegion(Region region, Array<Object> names, Array<Object> values) {
		...
		private void sortRegionsByIndex() {
			regions.sort(Comparator.comparingInt(region -> {
		...
		static public class Page {
			public String name;
			/**
			 * May be null if this page isn't associated with a file. In that case,
			 * {@link #texture} must be set.
			 */
	...
	static public class AtlasRegion extends TextureRegion {
		 ...
		 * stored as the index and is not considered as
		 * part of the sprite's name. This is useful for keeping animation frames in
		 * order.
		 * 
		 ...
		 * excluded: underscores denote special
		 ...
		 * image, after whitespace was removed for
		 * packing.
		 */
		 ...
		 * Most atlas region handling deals only with
		 * 0 or 90 degree rotation (enough to handle rectangles). More advanced texture
		 * packing may support other rotations (eg, for
		 * tightly packing polygons).
		 */
		public int degrees;

		/**
		 * Names for name/value pairs other than the fields provided on this class, each
		 * entry corresponding to {@link #values}.
		 */
		public @Null String[] names;

		/**
		 * Values for name/value pairs other than the fields provided on this class,
		 * each entry corresponding to {@link #names}.
		 */
		public @Null int[][] values;
		...
		@Override
		/**
		 * Flips the region, adjusting the offset so the image appears to be flipped as
		 * if no whitespace has been removed for
		 * packing.
		 */
		...
		/**
		 * Returns the packed width considering the {@link #rotate} value, if it is true
		 * then it returns the packedHeight,
		...
		/**
		 * Returns the packed height considering the {@link #rotate} value, if it is
		 * true then it returns the packedWidth,
	 ...
	 * is automatically positioned as if whitespace
